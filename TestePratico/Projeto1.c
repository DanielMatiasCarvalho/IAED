...
int verificarIDVoo(char *id)
{
    ...
    for(i=0;i<len;i++){
        ...
        /*faltava o else pois entrava no ele do if abaixo quando a primeira letra
        tava bem*/
        /*else*/if(i>=2 && i<4 && ((id[i]>='A' && id[i]<='Z') || 
            (id[i]>='a' && id[i]<='z'))){
                count++;
        }
        else{
                if(id[count]<'1' || id[count]>'9'){
    ...
    if (count==len){ 
        sem numeros, o len fica igual ao numero de letras
        return FALSE;
    }*/
    return TRUE;
}
...
partidas (comando 'p') e outra para as chegadas (comando 'c') ou, caso 
...
void listaVoosChegada(char *id)
{
    ...
    for (i=0;i<numVoo;i++){
        ptr=&tabelaVoo[i];
        aux=chegada(ptr);
        auxD=calcularData(aux.dataChg);
        if (!strcmp(id,tabelaVoo[i].idchg) && auxD>=dataatual){
            vooAer[len]=aux;
            len++;
        }
    }
